 ...
 * Time: 下午5:53
 */
public class DtmfTokenizer extends Tokenizer {
    ...
    public DtmfTokenizer(Reader reader, String padding_char, String first_letter) {
        this(reader, DEFAULT_BUFFER_SIZE);
        this.padding_char = padding_char;
        this.first_letter = first_letter;
    }

    public DtmfTokenizer(Reader input, int bufferSize) {
        super(input);
    ...
    public final boolean incrementToken() throws IOException {
        ...
        if (!done) {
            ...
            for (int i = 0; i < str.length(); i++) {
                char c = str.charAt(i);
                if (c < 128) {
                    String eng = DTMFEncoder.encode((""+c));
                    if (eng.length() > 0) {
                        stringBuilder.append(eng);
//                        firstLetters.append(eng.charAt(0));
                    }
                } else {
                    try {
                        ...
                        if (strs != null) {
                            for (int n = 0; n < strs.length; n++) {
                                strs[n] = DTMFEncoder.encode(strs[n]);
                            }
                            //get first result by default
                            String first_value = strs[0];
                            //TODO more than one dtmf
